...
if (isEmpty)
{
    ...
    var csvPath = Path.Combine(GetProjectRoot(), "Data", "books.csv"); // Data (mayúscula)
...
else
{
    ...
    var dataDir = Path.Combine(GetProjectRoot(), "Data"); // Data (mayúscula)
...
static async Task LoadCsvAsync(LibraryContext db, string csvPath)
{
    ...
    while (!parser.EndOfData)
    {
        ...
        foreach (var tagName in tagItems)
        {
            ...
            // Evitar vínculo duplicado (en memoria)
...
static async Task ExportTsvAsync(LibraryContext db, string dataDir)
{
    ...
    // ✅ JOINs explícitos (compatibles con SQLite; evita APPLY)
    var rows = await (
...
static string GetProjectRoot()
{
    ...
    while (d != null)
    {
        if (d.GetFiles("*.csproj").Any()) return d.FullName;
        d = d.Parent;
    }
